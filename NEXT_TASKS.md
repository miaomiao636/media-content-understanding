...
- 检查工作区差异，只保留配置、下载限制、缓存生命周期、测试和对应文档修改。
- 在 macOS 本机验证完成后，由 CI 再验证 Windows、Ubuntu 和 Python 3.9/3.11/3.13。
- 用户确认后再提交、推送并创建 `v0.2.1` 标签；不得直接覆盖现有 `v0.2.0`。
- 发布说明应明确 `keep_source_media` 只保留在受控缓存中，仍受 TTL 和容量策略管理。

验收标准：本地检查、远端 CI 和真实平台手工验收均通过，版本号在 Skill、Python 包、锁文件和 CHANGELOG 中一致。
...
- 为 `vision.max_upload_mb` 定义与 provider 专属限制的合并规则。
- 实现或删除 `verification_mode=low-confidence`；若实现，需要定义“低置信度”的可测试结构，而不是依赖自由文本猜测。
- 明确 `max_visual_calls` 同时限制转写分段、摘要和复核时的预算分配。
