...
> 工作区版本：`0.2.1`；最新 Git 标签仍为 `v0.2.0`
...
- 外部视觉路由在第一个有效 provider 返回后停止；配置中的低置信度二次复核尚未在路由器中实现。
- `analyze` 只有在输出包验证通过后才按配置清理当前任务；失败任务按独立保留时间保存，`acquire` 因需交付来源文件而保留成功任务。

### 推理

- 当前最准确的产品阶段是“正在准备发布的公开视频理解 Skill 0.2.1”，而不是完全无人值守的最终产品。
- 输出状态保持 `partial` 与 `SKILL.md` 中要求宿主 Agent 完成视觉复核的流程一致，应视为当前架构选择，而不是校验失败。

### 风险与边界

- 平台页面和风控会变化，真实链接可用性不能由离线测试长期保证。
- `vision.max_upload_mb`、低置信度二次复核和统一视觉调用预算仍未完整实现。详情见 `NEXT_TASKS.md`。
...
- `v0.2.0`：当前标签，指向 `9b61c48`。
- 工作区 `0.2.1`：完成配置优先级、下载上限、任务状态、缓存生命周期和专用浏览器登录档案；尚未提交、打标签或推送。
