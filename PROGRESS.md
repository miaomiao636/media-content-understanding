...
当前处于工作区 `0.2.1` 修复、真实平台验收和发布审查完成，等待远端 CI 与正式发布的阶段。最新已发布标签仍为 `v0.2.0`。
...
- 当前提交 HEAD 仍为 `9b61c48`，与 `origin/main` 对齐；工作区 `0.2.1` 修改尚未提交或推送。
- 已成功构建 `0.2.1` 的 wheel 和 sdist，并确认 wheel 包含统一 CLI、清理模块和 Keychain Swift 助手。

## 本次实际验证

2026-08-29 在迁移后的目录执行：
...
- GitHub Actions 最近一次远端运行结果未从本地仓库核实；本地等价测试已通过。
...
影响：修改业务代码、测试、文档、Context 和本机非敏感用户配置，但没有修改密钥或 Git 历史，也没有提交、打标签或推送。本机专用档案已保存用户主动建立的登录状态；两份临时复用测试媒体已移入废纸篓，可恢复。
